...
def timer():
    global game_timer, level, level_transition
    if game_timer >= 0 and level <= 3:
        game_timer += - timer_decrement
    elif game_timer < 0 and level < 3:
        level += 1
        level_transition += 1
...
def update():  # main update function
    ...
    if score >= 0 and level >= 1:
        if level_transition == 1:  # level 1 title
            BACKGROUND_IMG = "game_background1"
            if keyboard.RETURN == 1:
                level_transition = 2
        if level_transition == 2:  # level 1 gameplay
            timer()
            BACKGROUND_IMG = "game_background1"
            updatePlayer()  # calling our player update function
            updateJunk()  # calling junk update function
        if level_transition == 3:  # level 2 title
            BACKGROUND_IMG = "game_background2"
            if keyboard.RETURN == 1:
                level_transition = 4
        if level_transition == 4:  # level 2 gameplay
            timer()
            BACKGROUND_IMG = "game_background2"
            updatePlayer()
            updateJunk()
            updateSatellite()
        if level_transition == 5:  # level 3 title
            BACKGROUND_IMG = "game_background3"
            if keyboard.RETURN == 1:
                level_transition = 6
        if level_transition == 6:  # level 3 gameplay
            timer()
            BACKGROUND_IMG = "game_background3"
            updatePlayer()
            updateJunk()
...
def draw():
    ...
    if level >= 1:
        ...
        show_time = "Time: " + str(int(game_timer))
        screen.draw.text(show_time, topleft=(850, 15), fontsize=35, color="white")
